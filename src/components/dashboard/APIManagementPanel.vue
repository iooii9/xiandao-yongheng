...
const testAPI = async (api: APIConfig) => {
  ...
  try {
    ...
    const testPrompt = api.forceJsonOutput
      ? '你正在进行API连通性测试。请按照以下JSON格式输出测试结果：\n\n示例JSON格式：\n{"status": "ok", "message": "仙道永恒本-连通测试-OK"}\n\n请严格按照上述JSON格式输出。'
      : '你正在进行API连通性测试。请仅输出：仙道永恒本-连通测试-OK';
    ...
    if (api.forceJsonOutput) {
      try {
        ...
        ok = jsonResponse.status === 'ok' ||
             (jsonResponse.message && jsonResponse.message.includes('仙道永恒本')) ||
             response.toLowerCase().includes('ok');
      } catch {
        // JSON解析失败，尝试普通文本匹配
        ok = response.toLowerCase().includes('仙道永恒本') || response.toLowerCase().includes('ok');
      }
    } else {
      ok = response.toLowerCase().includes('仙道永恒本') || response.toLowerCase().includes('ok');
    }
...
const handleExport = () => {
  ...
  link.href = url;
  link.download = `仙道永恒-API配置-${new Date().toISOString().split('T')[0]}.json`;
  link.click();
